...
bilateral_filtered_image = cv2.bilateralFilter(norm_image.astype('float32'), int(.023*row), int(.019*row), int(.019*row))
...
for cnt in contours:
    ...
    if cv2.contourArea(cnt) < .01*row*col and cv2.contourArea(cnt) > .0005*row*col:
        print("Cell Detected")
        cv2.drawContours(contour_image, cnt, -1, (100,140,204), 3)
        M = cv2.moments(cnt)
        cx = int(M['m10']/M['m00'])
        cy = int(M['m01']/M['m00'])
